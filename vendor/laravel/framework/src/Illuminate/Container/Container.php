...
class Container implements ArrayAccess, ContainerContract
{
    ...
    /**
     * The container's bindings.(这里存放的是普通服务)
     *
     * @var array
     */
    protected $bindings = [];
    ...
    /**
     * The container's shared instances.(这里存放的是单例服务)
     *
     * @var array
     */
    protected $instances = [];
    ...
    /**
     * Determine if the given abstract type has been resolved.(判断这个抽象类是否是被解析过了)
     *
     * @param  string  $abstract
     * @return bool
     */
    ...
    public function bind($abstract, $concrete = null, $shared = false)
    {
        ...
        if (! $concrete instanceof Closure) {
            $concrete = $this->getClosure($abstract, $concrete); //这个函数的作用是将不是回调的变成回调，然后下面绑定到服务容器中
        }
        //compact的作用是将concrete和shared对应的变量组成一个数组
        $this->bindings[$abstract] = compact('concrete', 'shared');

        // If the abstract type was already resolved in this container we'll fire the
        // rebound listener so that any objects which have already gotten resolved
        // can have their copy of the object updated via the listener callbacks.
        //如果抽象类型已经被解析到服务容器中，将会执行rebound监听，这个rebound用来更新已经被解析的抽象类型对象副本(暂时不理解有啥用)
