...
class Application extends Container implements ApplicationContract, HttpKernelInterface
{
    ...
    {
        //这一步是注册基础的路径服务绑定,比如path,path.base等
        if ($basePath) {
            $this->setBasePath($basePath);
        }

        //这一步是注册基础绑定，绑定app和Container::class，这两个的具体实现都是$this（即Application.php的实例）
        $this->registerBaseBindings();

        //这一步是注册基础服务提供者,包括事件，日志，路由等服务提供者
    ...
    {
        static::setInstance($this); //这一步是设置全局的静态变量$instance为$this;
    ...
    public function register($provider, $options = [], $force = false)
    {
        ...
        $this->markAsRegistered($provider); //这个的作用是标记这个服务提供者已经被注册了

        // If the application has already booted, we will call this boot method on
        // the provider class so it has an opportunity to do its boot logic and
        // will be ready for any usage by this developer's application logic.
        //如果这个application已经启动了，那么就会执行服务提供者中的boot方法,从而完成boot中的相关逻辑
        if ($this->booted) {
    ...
    /**
     * Boot the application's service providers.(启动服务提供者，之前是注册了服务提供者)
     *
     * @return void
     */
